...
def train(train_loader, model, optimizer, epoch, save_path, writer):
    ...
    try:
        for i, (images, gts) in enumerate(train_loader, start=1):
            ...
            loss_init = wce_loss(preds[0], gts) + wce_loss(preds[1], gts) + wce_loss(preds[2], gts)
            loss_final = wce_loss(preds[3], gts)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str,
                        default='./snapshot/1013_PraNet_wce/', help='the path to save model and log')
